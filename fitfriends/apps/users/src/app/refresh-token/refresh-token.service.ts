...
export class RefreshTokenService {
  constructor(
    ...
    @Inject (jwtOptions.KEY) private readonly jwtMainConfig: ConfigType<typeof jwtOptions>,
  ) {}

  public async createRefreshSession(payload: RefreshTokenPayload) {
    const count = +this.jwtMainConfig.refreshTokenExpiresIn.slice(0, -1);
    const unit  =  this.jwtMainConfig.refreshTokenExpiresIn.at(-1);
